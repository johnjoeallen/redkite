...
| `ValidationRunner` | Runs `mvn clean install -Denforcer.skip=true` (`-DskipTests` by default, see `ValidationOptions.skipTests`; or `clean verify`/`clean test` — see `ValidationOptions.mode`) and, in `Mode.RUN`, optionally `spring-boot:run` to validate a project build |
