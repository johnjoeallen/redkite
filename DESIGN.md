...
| `ValidationRunner` | Runs `mvn clean install/verify/test -Denforcer.skip=true` (goal from `ValidationOptions.mode`; `-DskipTests` unless `ValidationOptions.enableTests`, uniformly across all three; `--no-transfer-progress` unless `ValidationOptions.fullLogs`) and, in `Mode.RUN`, optionally `spring-boot:run` to validate a project build |
...
| `ProjectConfigFile` | Reads a project's own `.redkite/settings.yml` (or `settings.yaml`) — see Apply Changes Flow below |
...
projects
  ...
  validation_maven_args varchar    ← unused since settings.yml replaced it; column kept, never read/written
  validation_env varchar           ← unused since settings.yml replaced it; column kept, never read/written
...
Enforcer is skipped because enforcer violations are what RedKite is fixing — running it during validation would create an unresolvable catch-22 on broken projects. Tests are skipped by default for speed, but this is configurable per-project (`mode`'s combination, below) — the shown command is `Mode.RUN`'s default.

Both the build and the `spring-boot:run` startup check accept extra configuration, read from the project's own `.redkite/settings.yml` (or `settings.yaml`, which takes precedence if both exist; `ProjectConfigFile`, red-kite-maven) — real YAML, parsed with SnakeYAML (`org.yaml:snakeyaml`) into its natural `Map<String, Object>` tree rather than a hand-rolled parser, then read field-by-field with each accessor tolerating a missing section or unexpected shape. Everything lives under `redkite.maven` — nested under a `redkite:` root rather than `maven:` directly, leaving room for future non-Maven config sections alongside it: `args` (a real YAML list, or a whitespace-separated scalar string for convenience), `profile`, `mode`, `fullLogs`, an `env` map, and a nested `spring.profiles`.

`mode` is a combination of `run`/`verify`/`test` tokens — a single scalar (`mode: verify`), a real YAML list (`mode: [run, test]`), or a comma/whitespace-separated scalar (`mode: run, test`); see `ProjectConfigFile.parseModeCombination`. From the resulting `Set<ValidationRunner.Mode>`, `ProjectConfigFile.parse` derives two independent things: the effective `ValidationRunner.Mode` — whichever of `RUN`/`VERIFY`/`TEST` is deepest in the set, since Maven's own lifecycle already nests them (`install` ⊇ `verify` ⊇ `test`), falling back to `RUN` if the set is empty or every token was unrecognized — and `ProjectConfig.enableTests()` — simply whether `TEST` is in the set, applied uniformly by `ValidationRunner.validate` regardless of which `Mode` was picked (`-DskipTests` is added unless `enableTests`). `Mode.RUN` (`mvn clean install`) is the only one that ever attempts the `spring-boot:run` startup check, and only when `spring-boot-maven-plugin` is present. `Mode.TEST` can only be selected when `test` was itself a token, so `enableTests` is always `true` in that case — `mvn clean test -DskipTests` never happens through the parser. `mode: run` alone (the default) matches today's existing behavior exactly (`mvn clean install -DskipTests`); `mode: [run, test]` runs the same `install` goal with tests on; `mode: verify` alone is a deliberate behavior change — it now also skips tests by default, where earlier it never did — write `mode: [verify, test]` for that traditional behavior.

`fullLogs` (default `false`) applies to every `mvn` invocation regardless of mode — `ValidationRunner.buildCommand` normally adds `--no-transfer-progress`; `fullLogs: true` omits it, so dependency download/upload activity shows up in the raw output.

These are bundled into `ValidationRunner.ValidationOptions(mavenArgs, env, mode, springBootArgs, enableTests, fullLogs)`, loaded fresh from `projectRoot` at the start of each apply job and passed into `ValidationRunner.validateWithStartup(..., options)` for both PRE_VALIDATE and POST_VALIDATE. This exists because projects that require an active Spring profile or environment-specific config to build/start would otherwise always fail startup validation, and different projects need different values. The project page shows a read-only panel with whatever the file currently resolves to — there's no UI form or API endpoint to edit it; the old `projects.validation_maven_args`/`validation_env` columns and their `/api/projects/{id}/validation` endpoint are gone from the code (columns remain in the schema, unused, per the project's no-destructive-migration convention). If a project has neither `.redkite/settings.yml` nor `settings.yaml`, `ProjectConfigFile.ensureDefaultExists` creates a fully commented-out `settings.yml` the first time RedKite scans it (called from `handleApiScan` right after `store.reconfigureForProject`) — idempotent and best-effort, so it never overwrites an existing file (even an empty one) and never blocks a scan on a write failure.
