    ...
    public List<MetadataResult> evaluateVersionMetadata(String scanId, List<ScanComponent> components, boolean offlineMode) {
        ...
        for (ScanComponent component : components) {
            if (component.snapshot()) {
                ...
                results.add(new MetadataResult(scanId, component.id(), MetadataType.VERSION, "none", component.version(), "unknown", "unknown", List.of(), List.of(), false, MetadataStatus.NOT_APPLICABLE, CacheState.MISSING, null, null, now, null, "SNAPSHOT dependencies are not scanned for version currency."));
                continue;
            }
            ...
            results.add(new MetadataResult(scanId, component.id(), MetadataType.VERSION, metadata.source(), component.version(), metadata.latestVersion(), metadata.latestSameMajorVersion(), metadata.upgradePathVersions(), metadata.allStableVersions(), freshness.complete(), freshness.status(), freshness.cacheState(), metadata.checkedAt(), metadata.checkedAt().plus(Duration.ofHours(24)), now, MetadataPolicy.staleUntil(metadata.checkedAt(), Duration.ofHours(24), Duration.ofDays(7)), freshness.complete() ? "Fresh version metadata" : "Stale version metadata"));
        }
        return results;
    }
    ...
    public List<MetadataResult> evaluateVulnerabilities(String scanId, List<ScanComponent> components, boolean offlineMode) {
        ...
        for (ScanComponent component : components) {
            if (component.snapshot()) {
                ...
                results.add(new MetadataResult(scanId, component.id(), MetadataType.VULNERABILITY, "none", component.version(), "unknown", "unknown", List.of(), List.of(), false, MetadataStatus.NOT_APPLICABLE, CacheState.MISSING, null, null, now, null, "SNAPSHOT dependency cannot be verified against stable Maven/CVE metadata."));
                continue;
            }
            ...
            results.add(new MetadataResult(scanId, component.id(), MetadataType.VULNERABILITY, "osv", component.version(), "unknown", "unknown", List.of(), List.of(), complete, MetadataStatus.FRESH, CacheState.FRESH, now, now.plus(Duration.ofHours(12)), now, null, vulns.isEmpty() ? "No CVEs found in available metadata; metadata complete." : "Vulnerabilities found"));
        }
        return results;
    }
}
