...
The `env` section of `.redkite/project.cfg` is a set of `KEY: value` pairs set on the processes RedKite spawns to validate this project — both the build check and, for Spring Boot projects, the `spring-boot:run` startup check.
