...
- **`args`** — the raw arguments, appended as-is. Use this for anything not covered by `profile` or `spring.profiles`. Written as a real YAML list, one argument per entry — the form to reach for when an argument's own value needs to contain a space (e.g. `-Dmessage=hello world`), since each list entry is unambiguous. A single whitespace-separated scalar string (`args: -Dfoo=bar -Dbaz=qux`) also works, split the same way, for anything simpler.
...
A Spring Boot active profile is a separate, related concept — see the `spring.profiles` field on [Build Validation](build-validation.md), which turns into `-Dspring-boot.run.profiles=<value>` but, unlike these two fields, only applies to the `spring-boot:run` startup check, never the plain build.
